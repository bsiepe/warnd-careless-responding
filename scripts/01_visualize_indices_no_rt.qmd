...
We could still compute the overlap with the response time indices, as we only excluded them from the hurdle approaches and the LPA, but they can still be computed on the raw data. However, for better interpretation, we remove them here as well.
...
similarity_plot_no_rt <- jaccard_sim_matrix |>
  ...
  filter(!Var1 %in% c("flag_average_response_time", "flag_sd_response_time")) |>
  pivot_longer(-Var1, names_to = "Var2", values_to = "Similarity") |>
  filter(!Var2 %in% c("flag_average_response_time", "flag_sd_response_time")) |>
  mutate(
    Var1 = case_when(
...
# exclude reaction time indices from similarity matrix
# before computing mean
# identify indices that do not contain "response_time"
keep_indices <- grep("response_time", rownames(jaccard_sim_matrix), 
                     invert = TRUE, value = FALSE)

# keep those filtered columns
filtered_matrix <- jaccard_sim_matrix[keep_indices, keep_indices]

# Calculate mean similarities (excluding diagonal)
mean_similarities <- sapply(1:ncol(filtered_matrix), function(i) {
  col_vals <- filtered_matrix[, i]
  mean(col_vals[-i])  # exclude diagonal element
})
names(mean_similarities) <- colnames(filtered_matrix)

# main similarity data
similarity_data <- filtered_matrix |>
  as.data.frame() |>
  rownames_to_column(var = "Var1") |>
  filter(!Var1 %in% c("flag_average_response_time", "flag_sd_response_time")) |>
  pivot_longer(-Var1, names_to = "Var2", values_to = "Similarity") |>
