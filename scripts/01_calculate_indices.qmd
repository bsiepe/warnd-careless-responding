...
1. Different cutoffs for individual indices
2. Different combination rules for multiple indices

We define a grid of different cutoffs: 
...
We then define different combination rules that we will apply to this grid: 
```{r}
...
integer_plot_pct <- df_indices |> 
  ...
  scale_x_continuous(breaks = 0:11, expand = c(0, 0), limits = c(-.6,11.1)) +
  theme_bs() +
  labs(x = "", 
       y = "Percentage",
       caption = "Removed some small values")  
integer_plot_pct
```
