...
flag_list <- lapply(careless_list, function(x){
    x |> 
    rowwise() |> 
    dplyr::mutate(flag_sum = sum(flag_sd_within, flag_average_response_time, flag_sd_response_time, flag_longstring, flag_mode, na.rm = TRUE)) |> 
...
compute_correlations <- function(flag_list, data) {
 imap(flag_list, function(flag_df, i) {
   ...
   excluded_ids <- flag_df |> 
     filter(flag_sum != 0) |> 
     select(external_id, counter)
   
   excluded_data <- data |> 
     inner_join(excluded_ids, by = c("external_id", "counter"))
   
   list(
     correlation = cor(excluded_data$nervous_d, excluded_data$overwhelm_d, use = "pairwise.complete.obs"),
     n_excluded = nrow(excluded_data)
   )
 })
}
...
Here is a visual summary of the results across different specifications in the form of a specification curve:

```{r}
df_cors_spec <- readRDS(here::here("output", "df_cors.RDS"))
# Combine plots
correlation_plot <- plot_correlation(df_cors_spec)
specification_plot <- plot_specification(df_cors_spec)

mv_plot_cors <- plot_grid(correlation_plot, specification_plot, 
          ncol = 1, rel_heights = c(1, 2), axis = "b",
          align = "h")

ggsave("mv_plot_cors.pdf", mv_plot_cors, device = "pdf", path = here::here("figures"),
       height = 8, width = 10)
```
 ...
 imap(flag_list, function(flag_df, i) {
   ...
   df_indices_models <- lpa_res |> 
     ...
     mutate(flag_lpa = ifelse(LPA_profile <= 2, 0, 1),
            ...
            flag_sum = ifelse(flag_sum == 0, 0, 1)) |> 
   ...
   list(
     n_careless_sum = sum(flag_df$flag_sum != 0, na.rm = TRUE),
     n_careless_lpa = sum(df_indices_models$flag_lpa, na.rm = TRUE),
     n_careless_irt = sum(df_indices_models$flag_irt, na.rm = TRUE),
     jaccard_similarity_lpa = jaccard_sim_matrix[["flag_sum", "flag_lpa"]],
     jaccard_similarity_irt = jaccard_sim_matrix[["flag_sum", "flag_irt"]]
   )
 })
}
...
Visualize the with two specification curves in one: 
```{r}
# Combine plots
lpa_similarity_plot <- plot_similarity(df_jaccard_spec, "lpa")
irt_similarity_plot <- plot_similarity(df_jaccard_spec, "irt")
specification_plot_lpa_sim <- plot_specification(df_jaccard_spec, 
                                                 type = "similarity",
                                                 model = "lpa")
specification_plot_irt_sim <- plot_specification(df_jaccard_spec, 
                                                 type = "similarity",
                                                 model = "irt")

mv_plot_lpa_sim <- plot_grid(
  lpa_similarity_plot,
  specification_plot_lpa_sim,
  ncol = 1,
  rel_heights = c(1, 1, 2),
  align = "h",
  axis = "b"
...
mv_plot_irt_sim <- plot_grid(
  irt_similarity_plot,
  specification_plot_irt_sim,
  ncol = 1,
  rel_heights = c(1, 1, 2),
  align = "h",
  axis = "b"
