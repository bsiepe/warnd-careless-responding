...
While we cannot share the full data as of now, here is a description of the columns in the data: 

    -   All EMA items are suffixed with "_d", and their corresponding reaction times are prefixed with "rt_".
    -   All EMA items are on Likert scales from 1 to 7. 
    -   All reaction time columns are specific dates with milisecond precision, which we can then convert to differences between the items if we know their order.
    -   A codebook for all variables with additional information is provided in the online supplement of the manuscript.
