...
df_indices <- df_indices |> 
  mutate(longstring_prop = as.integer(longstring_proportion * length(ema_items)),
...
```{r specifications}
sd_within_cutoff <- c(0.6, 1)
mean_rt_cutoff <- c(0.5, 1)
sd_rt_cutoff <- c(0.5, 1)
longstring_cutoff <- c(7, 8)
synonym_cutoff <- c(1,2)
antonym_cutoff <- c(1,2)
mode_cutoff <- c(4,5)
...
We then compute the multiverse:
```{r}
...
df_indices |>   
  mutate(k_values = as.factor(k_values), 
         ...
         longstring_proportion = as.factor(longstring_proportion)) |> 
