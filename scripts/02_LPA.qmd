...
# Sensitivity Analysis: LPA without response time indices

## Defining and Running the Model

We repeat the analysis

```{r list indices sensitivity}
# provide index names as in data frame
listIndices <- list(
  assessment_sd ~ 1, 
  mahalanobis_dist ~ 1,
...
```{r list distributions sensitivity}
# provide distributions in the same order as the list of index names
listDist <- list( 
  gaussian("identity"),
  gaussian("identity"),
...
In the next step, we determine the number of profiles. We here consider 1 up to 5 states. We perform the analysis 20 times per model with 20 different random starting values. As mentioned above, this helps reduce the risk for the model to converge to a local maximum. For each LPA model, we select the solution with the highest log-likelihood among the 20 replications. Note that there is no specific recommendation for the number of starting values to use in LPA. However, if the log-likelihood values vary substantially across replications for the model(s) that you want to choose for further investigation, it may be advisable to increase the number of replications to obtain more stable results. Due to the random starting values, we first set a seed to ensure the results can be replicated. Additionally, we define an empty results list (`results` in the code) and an empty model fit list (`fit` in the code) as well as an empty log-likelihood vector (`loglik` in the code) in which we store all relevant output for the 20 analyses. We can determine how stable the results are by looking at the range of the log-likelihood values. Furthermore, we can extract the results for the solution with the best log-likelihood and continue with those.

```{r analysis depmix sensitivity, eval = FALSE}
# set a seed because of the random starting values
set.seed(2020)
...
```{r save results sensitivity, eval = FALSE}
save(logliks_all, file = "logliks_all_s.RData")
save(fit, file = "fit_s.RData")
save(results, file = "results_s.RData")
save(bics, file = "bics_s.RData")
save(logliks, file = "logliks_s.RData")
save(messages, file = "messages_s.RData")
```

```{r load results sensitivity, include = FALSE}
# not visible in quarto output; used to load the results without re-estimating them
load("logliks_all_s.RData")
load("fit_s.RData")
load("results_s.RData")
load("bics_s.RData")
load("logliks_s.RData")
load("messages_s.RData")
```
...
```{r compare solutions sensitivity}
# combine into a data frame
results <- data.frame(nprofiles = 1:5, 
                      bics = bics, 
                      messages = messages)
...
```{r parameters 3 profiles sensitivity}
# define the number of profiles
nprofiles <- 3
...
# -2 because consider neither of the two response time indices
means_3profs <- matrix(NA, nrow = (length(indices)-2), ncol = nprofiles)
nignore <- nprofiles+
  nprofiles*nprofiles # these first parameters can be ignored
count_profile <- 0
# get the means
for(k in 1:nprofiles){
  ...
  # adjust now because we have one index less
  count_profile = count_profile + 6 + 5 # 6 intercepts and 5 sd per profile
}

#summary(fit[[nprofiles]]) #just to check this

# -2 because consider neither of the two response time indices
sds_3profs <- matrix(NA, nrow = (length(indices)-2), ncol = nprofiles)
nignore <- nprofiles+nprofiles*nprofiles
count_profile <- 0
# get the means
for(k in 1:nprofiles){
...
means_3profs[-4,] <- means_3profs[-4,]*
  indices_sds[-c(4,5,6)]+
  indices_means[-c(4,5,6)]

# back-standardize the SDs of the continuous indices
sds_3profs <- sds_3profs[-c(4), ] * 
  indices_sds[-c(4, 5, 6)]
sds_3profs <- rbind(sds_3profs[1:3, ], rep(0, 3), sds_3profs[4:5, ])

# give names
row.names(means_3profs) <- indices[-c(5,6)]
colnames(means_3profs) <- paste("profile",1:3, sep = "")
row.names(sds_3profs) <- indices[-c(5,6)]
colnames(sds_3profs) <- paste("profile",1:3, sep = "")
...
```{r parameters 4 profiles sensitivity}
# define the number of profiles
nprofiles <- 4
...
# -2 because consider neither of the two response time indices
means_4profs <- matrix(NA, nrow = (length(indices)-2), ncol = nprofiles)
nignore <- nprofiles+
  nprofiles*nprofiles # these first parameters can be ignored
count_profile <- 0
# get the means
...
# -2 because consider neither of the two response time indices
sds_4profs <- matrix(NA, nrow = (length(indices)-2), ncol = nprofiles)
nignore <- nprofiles+nprofiles*nprofiles
count_profile <- 0
# get the means
for(k in 1:nprofiles){
...
sds_4profs <- sds_4profs[-c(4), ] * 
  indices_sds[-c(4, 5, 6)]
sds_4profs <- rbind(sds_4profs[1:3, ], rep(0, 4), sds_4profs[4:5, ])

# give names
row.names(means_4profs) <- indices[-c(5,6)]
colnames(means_4profs) <- paste("profile",1:4, sep = "")
row.names(sds_4profs) <- indices[-c(5,6)]
colnames(sds_4profs) <- paste("profile",1:4, sep = "")
...
```{r 3 profiles sensitivity, fig.width = 9, fig.height = 6}
# reshape the data for 3 profiles
input_long <- means_3profs_sorted %>%
  as.data.frame() %>%
  tibble::rownames_to_column("variable") %>%
...
```{r 3 profiles including sd sensitivity, fig.width = 9, fig.height = 6}
plot_data <- data.frame(
  Variable = rownames(means_3profs_sorted),
  Mean = means_3profs_sorted,
  SD = sds_3profs_sorted
...
Note that the antonym violation probabilities (with averages of 0.02, 0.05, and 0.00 for the three profiles, respectively) and assessment standard deviation (with averages of 1.47, 1.75, and 1.85 for the three profiles, respectively) do not show clear differences across profiles and are therefore not further used for interpretation.

-   [**Profile 1**]{style="color:#31688E;"} is the largest profile and is characterized by generally low indices, suggesting attentive responding.

-   [**Profile 2**]{style="color:#35B779;"} is characterized by relatively large Mahalanobis and robust PCA indices, indicating consistency violations, which may point towards careless responding.
...
```{r 4 profiles sensitivity, fig.width = 9, fig.height = 6}
# reshape the data for 4 profiles
input_long <- means_4profs_sorted %>%
  as.data.frame() %>%
  tibble::rownames_to_column("variable") %>%
...
```{r 4 profiles including sd sensitivity, fig.width = 9, fig.height = 6}
plot_data <- data.frame(
  Variable = rownames(means_4profs_sorted),
  Mean = means_4profs_sorted,
  SD = sds_4profs_sorted
...
-   [**Profile 1**]{style="color:#35B779;"} is characterized by relatively large Mahalanobis and robust PCA indices, similar to profile 2 in the 3-profile solution, indicating consistency violations that may be interpreted as careless responding.

-   [**Profile 2**]{style="color:#31688E;"} is the largest profile and is characterized by generally low indices, suggesting attentive responding (seemingly overlapping with Profile 1 in the previous solution).

-   [**Profile 3**]{style="color:#FDE725;"} is characterized by relatively high long string and mode indices, similar to profile 3 in the 3-profile solution, suggesting aberrant response patterns, hinting towards careless responding.

-   [**Profile 4**]{style="color:#440154;"} is the smallest profile and characterized by even higher long string and mode indices, but overall also similar to profile 3 in the 3-profile solution, suggesting aberrant response patterns, hinting towards careless responding.

We conclude that adding a fourth profile is not substantively interesting because profiles 3 and 4 are similar. Next, for the chosen 3-profile solution, we will extract the modal profile assignments and attach them to the dataset. Since the profile order has changed when re-sorting the profiles by the long string index, we must apply the updated order when attaching the assignments.

```{r}
# attach the assignments to the dataset
data$LPA_profile <- fit[[3]]@posterior[,1]

# check the order that we used for plotting
longstring_order_3profs
```

```{r}
# all profiles have to be swapped
data$LPA_profile <- dplyr::recode(
  data$LPA_profile,
  `1` = 3,
  `2` = 1,
  `3` = 2
...
```{r, save data with assignments sensitivity, eval = FALSE, include = FALSE}
saveRDS(data[,c("external_id","counter","LPA_profile")], file = "data_profiles_without_time_indices.RDS")
```

To verify that we did not make a mistake, we check if the final proportions align with the estimated profile sizes of the 3-Profile solution, keeping in mind that small differences may result from classification uncertainty because not all observations are assigned to the profiles with probabilities of 1 or 0.
...
The probabilities are indeed approximately the same. 44 percent of the observations are categorized as attentive based on our interpretation of the profiles, about 6 percent points more than before when including average response time as index and choosing the 4-profile solition. As before, it is important to note that responses labeled as aberrant inconsistent are defined only relative to the other observed patterns, which entails ambiguity. Given the large proportions of these supposedly careless profiles, it is quite possible that meaningful substantive variation in response patterns is being misclassified as careless responding under this approach

```{r save profile plot sensitivity, eval = FALSE, include = FALSE}
input_long <- means_3profs_sorted %>%
  as.data.frame() %>%
  tibble::rownames_to_column("variable") %>%
  pivot_longer(cols = -variable, names_to = "profile", values_to = "mean") %>%
...
pdf("profiles_labels_s.pdf", width = 8, height = 6) 
ggplot(input_long, aes(x = variable, y = mean, color = profile)) +
  geom_point(size = 4, position = position_dodge(width = 0.5)) +
  scale_color_manual(values = c("#31688E", "#35B779", "#FDE725"),
                     labels = c("1 (attentive; 44%)", "2 (inconsistent; 19%)", "3 (invariable; 37%)")) +
  scale_x_discrete(labels = c("longstring_count" = "long string count", 
                              "mode_count" = "items that fall at the mode", 
                              "assessment_sd" = "within-assessment response SD", 
                              "mahalanobis_dist" = "Mahalanobis distance",
...
```{r save profile plot with means and sd sensitivity, eval = FALSE, include = FALSE}
pdf("profiles_mean_sd_s.pdf", width = 10, height = 7)
